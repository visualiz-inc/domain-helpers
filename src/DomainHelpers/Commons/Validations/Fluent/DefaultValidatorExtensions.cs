...
public static partial class DefaultValidatorExtensions {
    ...
    public static IRuleBuilderOptions<T, TProperty?> SetValidator<T, TProperty>(
        this IRuleBuilder<T, TProperty?> ruleBuilder, IPropertyValidator<T, TProperty> validator)
        where TProperty : struct {
        RuleComponentForNullableStruct<T, TProperty> component =
            new RuleComponentForNullableStruct<T, TProperty>(validator);
        RuleBuilder<T, TProperty?> rb = (RuleBuilder<T, TProperty?>)ruleBuilder;
    ...
    /// <param name="validator">The validator to set</param>
    /// <returns></returns>
    public static IRuleBuilderOptions<T, TProperty?> SetAsyncValidator<T, TProperty>(
        this IRuleBuilder<T, TProperty?> ruleBuilder, IAsyncPropertyValidator<T, TProperty> validator)
        where TProperty : struct {
        RuleComponentForNullableStruct<T, TProperty> component =
            new RuleComponentForNullableStruct<T, TProperty>(validator,
                validator as IPropertyValidator<T, TProperty>);
        RuleBuilder<T, TProperty?> rb = (RuleBuilder<T, TProperty?>)ruleBuilder;
        rb.AddComponent(component);
        return rb;
    }
    ...
    public static IRuleBuilderOptions<T, TProperty>
        Null<T, TProperty>(this IRuleBuilder<T, TProperty> ruleBuilder) {
    ...
    /// <returns></returns>
    public static IRuleBuilderOptions<T, TProperty>
        Empty<T, TProperty>(this IRuleBuilder<T, TProperty> ruleBuilder) {
    ...
    /// <param name="max"></param>
    /// <returns></returns>
    public static IRuleBuilderOptions<T, string> Length<T>(this IRuleBuilder<T, string> ruleBuilder,
        Func<T, int> min, Func<T, int> max) {
        return ruleBuilder.SetValidator(new LengthValidator<T>(min, max));
    }
    ...
    /// <returns></returns>
    public static IRuleBuilderOptions<T, string> Matches<T>(this IRuleBuilder<T, string> ruleBuilder,
        Func<T, string> expression, RegexOptions options) {
        return ruleBuilder.SetValidator(new RegularExpressionValidator<T>(expression, options));
    }
...
#pragma warning restore 618
    ...
    public static IRuleBuilderOptions<T, TProperty> NotEqual<T, TProperty>(
        ...
        return ruleBuilder.SetValidator(
            new NotEqualValidator<T, TProperty>(func, member, comparisonPropertyName, comparer));
    }
    ...
    public static IRuleBuilderOptions<T, string> NotEqual<T>(this IRuleBuilder<T, string> ruleBuilder,
        ...
        string comparisonPropertyName = GetDisplayName(member, expression);
        return ruleBuilder.SetValidator(new NotEqualValidator<T, string>(func, member, comparisonPropertyName,
            comparer));
    }
    ...
    /// <returns></returns>
    public static IRuleBuilderOptions<T, TProperty> Must<T, TProperty>(this IRuleBuilder<T, TProperty> ruleBuilder,
        Func<T, TProperty, ValidationContext<T>, bool> predicate) {
        predicate.Guard("Cannot pass a null predicate to Must.", nameof(predicate));
        return ruleBuilder.SetValidator(new PredicateValidator<T, TProperty>(
    ...
    public static IRuleBuilderOptions<T, TProperty> MustAsync<T, TProperty>(
        this IRuleBuilder<T, TProperty> ruleBuilder, Func<TProperty, CancellationToken, Task<bool>> predicate) {
        predicate.Guard("Cannot pass a null predicate to Must.", nameof(predicate));
    ...
    /// <returns></returns>
    public static IRuleBuilderOptions<T, TProperty> MustAsync<T, TProperty>(
        this IRuleBuilder<T, TProperty> ruleBuilder, Func<T, TProperty, CancellationToken, Task<bool>> predicate) {
        predicate.Guard("Cannot pass a null predicate to Must.", nameof(predicate));
    ...
    public static IRuleBuilderOptions<T, TProperty> LessThan<T, TProperty>(
        ...
        return ruleBuilder.SetValidator(new LessThanValidator<T, TProperty>(instance => {
            TProperty? nullable = func(instance);
    ...
    public static IRuleBuilderOptions<T, TProperty> GreaterThanOrEqualTo<T, TProperty>(
        ...
        where TProperty : IComparable<TProperty>, IComparable {
        MemberInfo member = valueToCompare.GetMember();
        Func<T, TProperty> func = AccessorCache<T>.GetCachedAccessor(member, valueToCompare);
        string name = GetDisplayName(member, valueToCompare);
    ...
    /// <returns></returns>
    public static IRuleBuilderOptions<T, TProperty> InclusiveBetween<T, TProperty>(
        this IRuleBuilder<T, TProperty> ruleBuilder, TProperty from, TProperty to, IComparer<TProperty> comparer) {
    ...
    /// <returns></returns>
    public static IRuleBuilderOptions<T, decimal> ScalePrecision<T>(this IRuleBuilder<T, decimal> ruleBuilder,
        int scale, int precision, bool ignoreTrailingZeros = false) {
        return ruleBuilder.SetValidator(
            new ScalePrecisionValidator<T>(scale, precision) { IgnoreTrailingZeros = ignoreTrailingZeros });
    }
    ...
    /// <returns></returns>
    public static IRuleBuilderOptionsConditions<T, TProperty> Custom<T, TProperty>(
        this IRuleBuilder<T, TProperty> ruleBuilder, Action<TProperty, ValidationContext<T>> action) {
        if (action == null) {
    ...
    /// <param name="action"></param>
    /// <returns></returns>
    public static IRuleBuilderOptionsConditions<T, TProperty> CustomAsync<T, TProperty>(
        this IRuleBuilder<T, TProperty> ruleBuilder,
        Func<TProperty, ValidationContext<T>, CancellationToken, Task> action) {
        if (action == null) {
            throw new ArgumentNullException(nameof(action));
        }

        return (IRuleBuilderOptionsConditions<T, TProperty>)ruleBuilder.MustAsync(
            async (parent, value, context, cancel) => {
    ...
    /// <returns></returns>
    public static IRuleBuilderOptions<T, IEnumerable<TElement>> ForEach<T, TElement>(
        ...
        Validator<IEnumerable<TElement>> innerValidator = new Validator<IEnumerable<TElement>>();

        // https://github.com/DomainHelpers.Core.Validations/DomainHelpers.Core.Validations/issues/1231
        ...
        IValidationRule<T, IEnumerable<TElement>> originalRule = DefaultValidatorOptions.Configurable(ruleBuilder);
        IRuleBuilderInitialCollection<IEnumerable<TElement>, TElement> collectionRuleBuilder =
            innerValidator.RuleForEach(x => x);
        ICollectionRule<IEnumerable<TElement>, TElement> collectionRule =
            DefaultValidatorOptions.Configurable(collectionRuleBuilder);
    ...
    /// <returns></returns>
    public static IRuleBuilderOptions<T, string> IsEnumName<T>(this IRuleBuilder<T, string> ruleBuilder,
        Type enumType, bool caseSensitive = true) {
    ...
    public static IRuleBuilderOptions<T, TProperty> ChildRules<T, TProperty>(
        this IRuleBuilder<T, TProperty> ruleBuilder, Action<Validator<TProperty>> action) {
        if (action == null) {
            throw new ArgumentNullException(nameof(action));
        }

        Validator<TProperty> validator = new Validator<TProperty>();
        string[] ruleSets = DefaultValidatorOptions.Configurable(ruleBuilder).RuleSets;
        action(validator);
        foreach (IValidationRuleInternal<TProperty> rule in validator.Rules) {
    ...
    /// <returns></returns>
    public static IRuleBuilderOptions<T, TProperty> SetInheritanceValidator<T, TProperty>(
        this IRuleBuilder<T, TProperty> ruleBuilder,
        Action<PolymorphicValidator<T, TProperty>> validatorConfiguration) {
        if (validatorConfiguration == null) {
            throw new ArgumentNullException(nameof(validatorConfiguration));
        }

        PolymorphicValidator<T, TProperty> validator = new PolymorphicValidator<T, TProperty>();
        validatorConfiguration(validator);
        return ruleBuilder.SetAsyncValidator(validator);
    }

    private static string
        GetDisplayName<T, TProperty>(MemberInfo member, Expression<Func<T, TProperty>> expression) {
        return ValidatorOptions.Global.DisplayNameResolver(typeof(T), member, expression) ??
               member?.Name.SplitPascalCase();
    }
}