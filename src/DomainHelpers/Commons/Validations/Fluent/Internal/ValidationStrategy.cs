...
public class ValidationStrategy<T> {
    ...
    public ValidationStrategy<T> IncludeProperties(params Expression<Func<T, object>>[] propertyExpressions) {
        if (_properties == null) {
            _properties =
                new List<string>(MemberNameValidatorSelector.MemberNamesFromExpressions(propertyExpressions));
        }
        else {
    ...
    private IValidatorSelector GetSelector() {
        ...
        if (_properties != null || _ruleSets != null || _customSelector != null) {
            ...
            if (_properties != null) {
                selectors.Add(
                    ValidatorOptions.Global.ValidatorSelectors.MemberNameValidatorSelectorFactory(
                        _properties.ToArray()));
            }
            ...
            if (_ruleSets != null) {
                selectors.Add(
                    ValidatorOptions.Global.ValidatorSelectors
                        .RulesetValidatorSelectorFactory(_ruleSets.ToArray()));
            }

            selector = selectors.Count == 1
                ? selectors[0]
                : ValidatorOptions.Global.ValidatorSelectors.CompositeValidatorSelectorFactory(selectors);
        }
        else {
    ...
    internal ValidationContext<T> BuildContext(T instance) {
        return new ValidationContext<T>(instance, null, GetSelector()) { ThrowOnFailures = _throw };
    }
}