...
internal class IncludeRule<T> : PropertyRule<T, T>, IIncludeRule {
    ...
    public IncludeRule(IValidator<T> validator, Func<CascadeMode> cascadeModeThunk, Type typeToValidate)
        ...
        ChildValidatorAdaptor<T, T> adaptor = new ChildValidatorAdaptor<T, T>(validator, validator.GetType());
    ...
    public IncludeRule(Func<ValidationContext<T>, T, IValidator<T>> func, Func<CascadeMode> cascadeModeThunk,
        ...
        : base(null, x => x, null, cascadeModeThunk, typeToValidate) {
        ChildValidatorAdaptor<T, T> adaptor = new ChildValidatorAdaptor<T, T>(func, validatorType);
        // Note: ChildValidatorAdaptor implements both IPropertyValidator and IAsyncPropertyValidator
        // So calling AddAsyncValidator will actually register it as supporting both sync and async.
        AddAsyncValidator(adaptor, adaptor);
    }
    ...
    public static IncludeRule<T> Create<TValidator>(Func<T, TValidator> func, Func<CascadeMode> cascadeModeThunk)
        ...
        return new IncludeRule<T>((ctx, _) => func(ctx.InstanceToValidate), cascadeModeThunk, typeof(T),
            typeof(TValidator));
    }

    public override async ValueTask ValidateAsync(ValidationContext<T> context, bool useAsync,
        ...
        // See https://github.com/DomainHelpers.Core.Validations/DomainHelpers.Core.Validations/issues/1989
        bool shouldAddStateKey =
            !context.RootContextData.ContainsKey(MemberNameValidatorSelector.DisableCascadeKey);
