...
namespace DomainHelpers.Core.Validations {
    public interface IValidationRule<T, out TProperty> : IValidationRule<T> {
        ...
        /// <param name="fallback">
        ///     A synchronous property validator to use as a fallback if executed synchronously. This parameter
        ///     is optional. If omitted, the async validator will be called synchronously if needed.
        /// </param>
        void AddAsyncValidator(IAsyncPropertyValidator<T, TProperty> asyncValidator,
            IPropertyValidator<T, TProperty> fallback = null);
    }
    ...
    public interface IValidationRule<T> : IValidationRule {
        ...
        /// <param name="applyConditionTo">
        ///     Whether the condition should be applied to the current property validator in the chain,
        ///     or all preceding property validators in the chain.
        /// </param>
        ...
        ///     or all preceding property validators in the chain.
        /// </param>
        void ApplyAsyncCondition(Func<ValidationContext<T>, CancellationToken, Task<bool>> predicate,
            ApplyConditionTo applyConditionTo = ApplyConditionTo.AllValidators);
